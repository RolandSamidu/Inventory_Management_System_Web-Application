						...
						<div class="card w-100">
							<div class="card-body p-4">
								<div
									class="d-sm-flex d-block align-items-center justify-content-between mb-9">
									<div class="mb-3 mb-sm-0">
										<label for="searchDate">Enter Date:</label> <input type="text"
											id="searchDate" placeholder="YYYY-MM-DD">
									</div>
									<div>
										<button type="button" class="btn btn-dark"
											onclick="searchData()">View Items</button>
									</div>
								</div>
								<div class="table-responsive row">
									<!-- Table 2 -->
									<table class="table text-nowrap mb-0 align-middle"
										id="searchResultsTable" style="display: none;">
										<thead class="text-dark fs-4">
											<tr>
										...
										<tbody>
											...
											try {
												Connection con = ConnectionProvider.getCon();
												Statement st = con.createStatement();
												String searchDate = request.getParameter("searchDate"); // Get the entered date
												String query = "SELECT receipt.pid, stockdetails.itemname, receipt.reitemqty, receipt.SubTotal FROM receipt INNER JOIN stockdetails ON receipt.itemno = stockdetails.itemno";
												if (searchDate != null && !searchDate.isEmpty()) {
													query += " WHERE DATE(receipt.date) = '" + searchDate + "'"; // Filter by the entered date
												}
									...
									try {
										...
										"SELECT SUM(receipt.SubTotal) FROM receipt INNER JOIN stockdetails ON receipt.itemno = stockdetails.itemno ");
										while (rs1.next()) {
							...
							<div class="card-body p-4">
								...
								<div class="table-responsive">
									...
									try {
										...
										"SELECT DATE(date) AS purchaseDate, SUM(totalprice) AS totalOnDay FROM purchase WHERE date >= CURDATE() - INTERVAL 7 DAY GROUP BY DATE(date) ORDER BY purchaseDate DESC"); // Replace with your query and column names
										...
										while (rs.next()) {
											xValues.add(rs.getString("purchaseDate")); // Replace with your column name for x-values
											yValues.add(rs.getInt("totalOnDay")); // Replace with your column name for y-values
